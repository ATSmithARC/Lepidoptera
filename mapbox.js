...
function renderListings(features) {
  ...
  if (features.length) {
    ...
    /*
    empty.textContent =
      "Use the polygon tool to search. Double-click to finish...";
    listingEl.appendChild(empty);
...
map.on("load", () => {
  ...
  map.on("click", "poi-Animalia", (e) => {
    //map.getCanvas().style.cursor = "pointer";
    const feature = e.features[0];
    const popup = new mapboxgl.Popup({/*offset: popupOffsets,*/ className: 'map-popup1'})
      .setLngLat(feature.geometry.coordinates)
  ...
  map.on("mouseleave", "poi-Plantae", () => {
    map.getCanvas().style.cursor = "";
    popup.remove();
  });
  map.on("mouseleave", "poi-Animalia", () => {
    map.getCanvas().style.cursor = "";
    popup.remove();
  });
  map.on("mouseleave", "poi-Fungi", () => {
    map.getCanvas().style.cursor = "";
    popup.remove();
  });
  map.on("mouseleave", "poi-Bacteria", () => {
    map.getCanvas().style.cursor = "";
    popup.remove();
  });
  map.on("mouseleave", "poi-Chromista", () => {
    map.getCanvas().style.cursor = "";
    popup.remove();
  });
  map.on("mouseleave", "poi-Incertae Sedis", () => {
    map.getCanvas().style.cursor = "";
    popup.remove();
  });
